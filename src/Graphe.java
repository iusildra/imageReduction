...
public class Graphe {
    ...
    // liste adj : dans l.get(i) il y a tous les successeurs de i avec la capacité
    // des arcs sortants
    // par ex, si on a l'arc i->j de capacité c, alors dans la liste l.get(i) on
    // trouvera le couple (j,c)

    private ArrayList<ArrayList<Couple<Integer, Integer>>> pred; // liste adj : pred dans l.get(i) arcs j->i
    // on maintient l'invariant que si (j,c) est dans l.get(i), alors (i,c) est dans
    // pred.get(j)
    ...
    public boolean equals(Object o) {
        ...
        int i = 0;
        while (i < getN() && ok) {
            ok = l.get(i).containsAll(graphe.l.get(i))
                    && graphe.l.get(i).containsAll(l.get(i))
                    && pred.get(i).containsAll(graphe.pred.get(i))
                    && graphe.pred.get(i).containsAll(pred.get(i));
            i++;
        }
        return ok;
    }
